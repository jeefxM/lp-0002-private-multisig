...
pub enum MsigInstruction {
    ...
    Approve {
        ...
        proposal_id: [u8; 32],
        /// The member's ML-KEM viewing public key. Since LEZ v0.2.1 the account id commits to
        /// the viewing key (`for_regular_private_account(npk, vpk, identifier)`), so the guest
        /// needs it to re-derive the rider id from the SAME `secret` (review item #6). Travels
        /// as part of the private instruction witness, like `secret` and `merkle_path`.
