...
/// `AccountId::for_regular_private_account(NullifierPublicKey::from(secret), vpk, VOTE_IDENTIFIER)`.
...
fn main() {
    ...
    let post_states = match instruction {
        ...
        MsigInstruction::Approve {
            ...
            approve(proposal, rider, secret, merkle_path, proposal_id, &vpk)
        }
...
mod msig6_binding_tests {
    ...
    /// A deterministic test vpk built through serde (the only guest-safe constructor:
    /// `from_bytes` is host-only). Serialization of the newtype is its inner `Vec<u8>`.
    fn test_vpk() -> ViewingPublicKey {
        let bytes = vec![0x5Au8; ViewingPublicKey::LEN];
        let words = risc0_zkvm::serde::to_vec(&bytes).expect("serialize raw vpk bytes");
        risc0_zkvm::serde::from_slice(&words).expect("deserialize as ViewingPublicKey")
    }
    ...
    fn proposal_account() -> AccountWithMetadata {
        ...
        let arbitrary_id =
            AccountId::for_regular_private_account(&NullifierPublicKey::from(&[1u8; 32]), &test_vpk(), 0);
    ...
    fn voting_id() -> AccountId {
        AccountId::for_regular_private_account(&NullifierPublicKey::from(&SECRET), &test_vpk(), VOTE_IDENTIFIER)
    }
    ...
    fn approve_accepts_live_rider_bound_to_member_secret() {
        ...
        let posts = approve(proposal_account(), rider, SECRET, path(), PID, &test_vpk());
    ...
    fn approve_rejects_default_rider() {
        ...
        let _ = approve(proposal_account(), rider, SECRET, path(), PID, &test_vpk());
    }
    ...
    fn approve_rejects_rider_not_keyed_by_secret() {
        let wrong_id = AccountId::for_regular_private_account(&NullifierPublicKey::from(&[0xFFu8; 32]), &test_vpk(), VOTE_IDENTIFIER);
